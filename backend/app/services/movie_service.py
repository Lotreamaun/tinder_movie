...
class MovieService:
    ...
    def get_movie_by_kinopoisk_id(db: Session, kinopoisk_id: int) -> Optional[Movie]:
        stmt = select(Movie).where(Movie.kinopoisk_id == kinopoisk_id)
        return db.execute(stmt).scalar_one_or_none()


    def list_movies(db: Session, limit: int = 50, offset: int = 0) -> Sequence[Movie]:
        stmt = select(Movie).order_by(Movie.created_at.desc()).limit(limit).offset(offset)
        return list(db.execute(stmt).scalars())


    def update_movie_active(db: Session, movie: Movie, is_active: bool) -> Movie:
