...
def list_user_swipes(db: Session, user_id: str, limit: int = 50, offset: int = 0) -> Sequence[UserSwipe]:
    stmt = select(UserSwipe).where(UserSwipe.user_id == user_id).order_by(UserSwipe.swiped_at.desc()).limit(limit).offset(offset)
    return list(db.execute(stmt).scalars())


def get_swipe(db: Session, user_id: str, movie_id: str, group_participants: list[int]) -> Optional[UserSwipe]:
